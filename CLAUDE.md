...
Per-user pods (e.g. propanes.myworkbench.ai/&lt;user&gt;) boot from the baked `/app` image build by default, but `docker-launcher-entrypoint.sh` supports running the services from the NFS-mounted source tree instead. Opt in by dropping a marker on the pod's persistent volume:
...
- **Floating overlays must have an opaque background.** Anything that floats over arbitrary content — context menus, dropdowns, popovers, tooltips with text, spotlight pickers, action popups (e.g. `.pty-ctx-menu`, `.status-dot-menu`, `.cos-thread-rail-popup`) — must use a hardcoded opaque color (`#1e293b` for the dark theme) **not** `var(--pw-bg-surface)`. The `--pw-bg-surface` token is overridden to a translucent tint (`rgba(255,255,255,0.04)`) inside dark scopes like `.cos-popout`, `.cos-thread-side`, `.cos-learnings-side` for subtle inline regions. CSS variables cascade through the DOM tree regardless of `position: fixed`, so a popup rendered as a child of those scopes inherits the translucent value and shows whatever's behind it. Test rule: if the element renders on top of *other* content (not just a sibling block), it needs solid paint. Same goes for `backdrop-filter: blur` — blur is decoration, not a substitute for opacity behind text.
