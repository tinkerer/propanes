...
export class JsonOutputParser {
  ...
  private parseJsonEventInner(obj: any): ParsedMessage[] {
    ...
    if (type === 'system') {
      // Compaction lifecycle: a "compacting" status marks the beginning, a
      // compact_boundary marks where the context was actually trimmed. Both
      // are typed system events — surface them so operators can tell why a
      // session's earlier context vanished.
    ...
    if (type === 'user') {
      // Transcript JSONLs persist the post-compaction summary as a user
      // message flagged isCompactSummary. It's harness-generated, not
      // operator input — render as a system line, not a "You" bubble.
      if (obj.isCompactSummary) {
        const summaryText = typeof obj.message?.content === 'string'
          ? obj.message.content
          : Array.isArray(obj.message?.content)
            ? obj.message.content.filter((b: any) => b.type === 'text').map((b: any) => b.text).join('\n')
            : '';
        return [{ id: this.genId(), role: 'system', timestamp: Date.now(), content: `Context compacted — summary:\n${summaryText}` }];
      }
      ...
      if (typeof content === 'string') {
        ...
        // Classify tool results before human input: a user envelope that
        // carries tool_result blocks is the harness returning tool output,
        // not the operator typing. Any text blocks riding along in the same
        // envelope (system-reminders, injected context) must therefore not
        // be emitted as user_input — they'd render as a "You" bubble the
        // operator never wrote. Surface them as system lines instead.
        const hasToolResult = content.some((b: any) => b.type === 'tool_result');
        const text = content.filter((b: any) => b.type === 'text').map((b: any) => b.text).join('\n');
        if (text) {
          results.push({
            id: this.genId(),
            role: hasToolResult ? 'system' : 'user_input',
            timestamp: Date.now(),
            content: text,
          });
        }
...
export function createOutputParser(permissionProfile: string): JsonOutputParser | TerminalOutputParser {
  // headless-yolo emits a one-shot stream-json pipe and the two stream
  // profiles (headless-stream-yolo / headless-stream-require) emit
  // bidirectional stream-json — parse all three with the JSON parser.
  if (
...
// Timestamp of a JSONL line: prefer the line's own ISO timestamp, fall back
// to "now" for live streams that don't stamp events. Shared by the Claude
// parsers here and the Codex classifier in codex-output-parser.ts.
export function tsOf(obj: any): number {
